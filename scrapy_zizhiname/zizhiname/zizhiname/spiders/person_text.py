...
class AllXinliangSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # redis_len = r.scard('title_renyuan')

        # logging.error('当前数据长度%s' % redis_len + '\n\n')
        # while redis_len != 0:
        #     for index in range(redis_len):
        #         if not index == 200:
        #             name = r.spop("title_renyuan").decode('utf8')
        #             try:
        my_name = '天津开发区广厦园林建筑工程有限公司'
        yield scrapy.FormRequest(self.url,
                                 formdata={'qy_name': my_name},
                                 callback=self.parse)
            ...
            #             AAA = '公司访问失败--第%s' % (index) + e + name
            #             logging.error('访问公司未响应--' + AAA + '\n\n')
            #             print('访问公司未响应--' + AAA + '\n\n')
            #     break
            # time.sleep(2)
    # 选择公司
