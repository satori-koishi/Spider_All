            ...
            for c in content:
                ...
                for t in td:
                    ...
                    if field_name == '诚信记录编号':
                        ...
                        merit['result'] = result
                        what_action = t.xpath('./div/span[1]/text()').extract_first()
                        what_action = what_action.replace('【', '')
                        what_action = what_action.replace('】', '')
                        merit['what_action'] = what_action
                        start_date = t.xpath('./div/span[2]/text()').extract_first()
                        start_date = start_date.slipt('：')[1]
                        merit['start_date'] = start_date
                        department = t.xpath('./div/a/@data-no').extract_first()
                        merit['department'] = department
                        d_content = t.xpath('./div/a/@data-text').extract_first()
