...
class ShanxiJianzhuImformationSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        page = Selector(response=response).xpath('//span[@id="RecordSetCountCtrl"]/text()').extract_first()
        page = int(page)+1
        print(page, 'pppppppppppppppppppppppppppppppp')
        self.index += 1
        if self.index != page:
                yield scrapy.Request(url='http://111.11.196.111/aspx/corpinfo/CorpInfo.aspx?'
                                         'corpname=&cert=&PageIndex=%s' % self.index,
                                         callback=self.parse)
