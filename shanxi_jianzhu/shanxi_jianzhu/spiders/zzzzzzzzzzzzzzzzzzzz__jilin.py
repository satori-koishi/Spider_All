...
class ShanxiJianzhuImformationSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if self.index != 130:
                ...
                now_time = time.time() * 1000 - 1000000
                yield scrapy.Request(url='http://cx.jlsjsxxw.com/handle/NewHandler.'
                                         'ashx?method=SwCorpData&CorpName=&AptitudeNum=&'
                                         'TradeID=&BoundID=&LevelID=&ProvinceNum=&nPageIndex'
                                         '=%s&nPageCount=127&nPageRowsCount=2516&nPageSize=20'
                                         '&_=%s' % (self.index, now_time), callback=self.parse)
    ...
    def company_information(self, response):
        ...
        register_person_url = 'http://cx.jlsjsxxw.com/handle/Company_OutDetails_CertifiedEngineers.ashx' \
                              '?corpid=%s&ReocrdGuid=%s&_=1556177544518' \
                              % (response.meta['cc'][0][1], response.meta['cc'][0][0])

        admininstration_prson_url = 'http://cx.jlsjsxxw.com/handle/Company_OutDetails_CertifiedEngineers.ashx' \
                              '?corpid=%s&ReocrdGuid=%s&_=1556177544518' \
                              % (response.meta['cc'][0][1], response.meta['cc'][0][0])

        system_person_url = 'http://cx.jlsjsxxw.com/handle/Company_OutDetails_CertifiedEngineers.ashx' \
                              '?corpid=%s&ReocrdGuid=%s&_=1556177544518' \
                              % (response.meta['cc'][0][1], response.meta['cc'][0][0])

        yield scrapy.Request(url=register_person_url,
                             callback=self.person_info,
                             meta={'company_name': company_name, 'licenseNum': number},
                             dont_filter=True
                             )
        yield scrapy.Request(url=admininstration_prson_url,
                             callback=self.person_info,
                             meta={'company_name': company_name, 'licenseNum': number},
                             dont_filter=True
                             )
        yield scrapy.Request(url=system_person_url,
                             callback=self.person_info,
                             meta={'company_name': company_name, 'licenseNum': number},
                             dont_filter=True
                             )
        ...
        if person_every['Title'] == '注册人员':
            for t in tr:
                person_data = {'companyName': response.meta['company_name'],
                               'licenseNum': response.meta['licenseNum'], 'name': '', 'area': '吉林省', 'sex': '',
